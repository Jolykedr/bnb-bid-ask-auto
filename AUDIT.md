...
### H4 ✅ `self.worker` перезаписывается без cleanup (FIXED 2026-05-09, commit 026ac02)
### H5 ✅ `_swap_worker` перезаписывается без cleanup (FIXED 2026-05-09, commit 026ac02)
> См. блок «pre-overwrite cleanup guards» внизу.
...
НОРМ
### M26 ✅ `_collect_worker` перезаписывается без cleanup (FIXED 2026-05-09, commit 026ac02)
> См. блок «pre-overwrite cleanup guards» внизу.
...
# ✅ ИСПРАВЛЕНО (pre-overwrite cleanup guards, 2026-05-09, commit 026ac02)

### H4 ✅ `self.worker` перезаписывается без cleanup (FIXED)
### H5 ✅ `_swap_worker` перезаписывается без cleanup (FIXED)
### M26 ✅ `_collect_worker` перезаписывается без cleanup (FIXED)

**Файл:** `ui/manage_tab.py` — 4 точки с pre-assignment guard:
- line 3188 (`BatchCloseWorker`) — H4a
- line 3278 (`BatchCollectWorker`) — M26
- line 3447 (`ClosePositionsWorker`) — H4b
- line 3941 (`SwapWorker`) — H5

**Было:** `self.worker = NewWorker(...)` напрямую перезаписывал ссылку. Если старый QThread ещё бежал (race на двойной клик / programmatic re-entry), Python терял ссылку → GC уничтожал живой QThread → `QThread::~QThread()` abort на выходе.
...
`_safe_cleanup_worker` уже существовал (line 1999-2010), идемпотентный (`isRunning()` guard), используется в 9 других местах. Если воркер ещё бежит — идёт в `_dying_workers`, дотикивает, чистится через `finished` signal. Если уже завершён — `deleteLater()` сразу.

**Тесты:** 1355 passed, 4 failed (тот же baseline — known N1).
**Регрессий:** 0 — guard no-op когда `self.worker is None`.
**Практический риск до фикса:** низкий (кнопки блокировались `setEnabled(False)`), но purely-defensive фикс закрывает racy paths.
